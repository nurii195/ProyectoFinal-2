...
public class GestorArticuloTest {
	...
	public void testBaja() {
		...
		articulo.setBaja(false);
		boolean alta = instancia.alta(articulo);
		boolean baja = instancia.baja(articulo);
		assertTrue(baja);

		// Dar de baja a un articulo que no esta
		Articulo articuloDos = new Articulo();
		boolean baja2 = instancia.baja(articuloDos);
		assertFalse(baja2);

		// Dar de baja a un articulo nulo
		Articulo articuloTres = null;
		boolean alta2 = instancia.alta(articuloTres);
		assertFalse(alta2);
		boolean baja3 = instancia.baja(articuloTres);
		assertFalse(baja3);
		assertNull(articuloTres);

		// Dar de baja un objeto que no es del tipo apropiado

		boolean alta3 = instancia.baja("hola");
		assertFalse(alta3);
